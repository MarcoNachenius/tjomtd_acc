...
func test_range_display_with_tower_range_aura():
    # ================================================================
    #                     ** CREATE BLANK MAP **
    # ================================================================
    ...
    # Test values
    const TEST_STARTING_RANGE: int = 200
    const TEST_STARTING_RANGE_DISPLAY_TEXT: String = "200"
    const TEST_RANGE_INCREASE_FACTOR: float = 0.2
    const TEST_BUFFED_RANGE: int = 240
    const TEST_BUFFED_RANGE_DISPLAY_TEXT: String = "200 (+40)"
    ...
    # Artificially assign tower's starting range
    black_marble_lvl_1.PRIMARY_PROJECTILE_SPAWNER.INITIAL_RANGE = TEST_STARTING_RANGE
    black_marble_lvl_1.PRIMARY_PROJECTILE_SPAWNER.__detection_range = TEST_STARTING_RANGE
    ...
    # Ensure display range without buff is correct
    var got_initial_display_text: String = test_random_tower_build_hud.SELECTED_TOWER_STATS_CONTAINER.TOWER_ATTR_CONTAINER.RANGE_VALUE.text
    assert_eq(got_initial_display_text, TEST_STARTING_RANGE_DISPLAY_TEXT, "Display range without buff amount is correct")

    # ADD RANGE AURA
    # ---------------
    # Instantiate range aura scene
    var test_range_aura: TowerRangeAura = TowerAuraConstants.TOWER_RANGE_AURA_PRELOAD.instantiate()
    # Set aura increase factor
    test_range_aura.RANGE_INCREASE_FACTOR = TEST_RANGE_INCREASE_FACTOR
    # Add range aura to map (where it would appear when implemented)
    test_game_map.add_child(test_range_aura)
    # Place range aura in same position as tower
    test_range_aura.global_position = black_marble_lvl_1.global_position
    # Process frame which adds range aura
    await get_tree().process_frame
    ...
    # Ensure range buff has reached tower
    assert_eq(black_marble_lvl_1.PRIMARY_PROJECTILE_SPAWNER.get_detection_range(), TEST_BUFFED_RANGE, "Tower range has been increased.")

    # Simulate tower being selected again after range buff has been applied
    test_game_map.tower_selected.emit(black_marble_lvl_1)
    # Process frame which handles signal emission
    await get_tree().process_frame

    # Ensure display range has been updated after range buff
    var got_buffed_RANGE_display_text: String = test_random_tower_build_hud.SELECTED_TOWER_STATS_CONTAINER.TOWER_ATTR_CONTAINER.RANGE_VALUE.text
    assert_eq(got_buffed_RANGE_display_text, TEST_BUFFED_RANGE_DISPLAY_TEXT, "Display range with buff amount is correct")

    # Remove range aura from map
    test_range_aura.queue_free()
    ...
    # Ensure tower's range has been restored to initial value
    assert_eq(black_marble_lvl_1.PRIMARY_PROJECTILE_SPAWNER.get_detection_range(), TEST_STARTING_RANGE)

    # Simulate tower being selected again after range buff has been removed
    test_game_map.tower_selected.emit(black_marble_lvl_1)
    # Process frame which handles signal emission
    await get_tree().process_frame

    # Ensure display range has been restored to original value
    got_initial_display_text = test_random_tower_build_hud.SELECTED_TOWER_STATS_CONTAINER.TOWER_ATTR_CONTAINER.RANGE_VALUE.text
    assert_eq(got_initial_display_text, TEST_STARTING_RANGE_DISPLAY_TEXT, "Display range without buff amount is correct")
