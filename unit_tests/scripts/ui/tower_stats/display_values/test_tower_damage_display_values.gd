...
func test_damage_display_with_tower_damage_aura():
    # ================================================================
    #                     ** CREATE BLANK MAP **
    # ================================================================
    ...
    # Test values
    const TEST_STARTING_DAMAGE: int = 200
    const TEST_STARTING_DAMAGE_DISPLAY_TEXT: String = "200"
    const TEST_DAMAGE_INCREASE_FACTOR: float = 0.2
    const TEST_BUFFED_DAMAGE: int = 240
    const TEST_BUFFED_DAMAGE_DISPLAY_TEXT: String = "200 (+40)"
    ...
    # Artificially assign tower's starting damage
    ...
    # Ensure display damage without buff is correct
    var got_initial_display_text: String = test_random_tower_build_hud.SELECTED_TOWER_STATS_CONTAINER.TOWER_ATTR_CONTAINER.DAMAGE_VALUE.text
    assert_eq(got_initial_display_text, TEST_STARTING_DAMAGE_DISPLAY_TEXT, "Display damage without buff amount is correct")

    # ADD DAMAGE AURA
    # ---------------
    # Instantiate damage aura scene
    var test_damage_aura: TowerDamageAura = TowerAuraConstants.TOWER_DAMAGE_AURA_PRELOAD.instantiate()
    # Set aura increase factor
    test_damage_aura.DAMAGE_INCREASE_FACTOR = TEST_DAMAGE_INCREASE_FACTOR
    # Add damage aura to map (where it would appear when implemented)
    test_game_map.add_child(test_damage_aura)
    # Place damage aura in same position as tower
    test_damage_aura.global_position = black_marble_lvl_1.global_position
    # Process frame which adds damage aura
    await get_tree().process_frame
    ...
    # Ensure damage buff has reached tower
    assert_eq(black_marble_lvl_1.PRIMARY_PROJECTILE_SPAWNER.get_damage(), TEST_BUFFED_DAMAGE, "Tower damage has been increased.")

    # Simulate tower being selected again after damage buff has been applied
    test_game_map.tower_selected.emit(black_marble_lvl_1)
    # Process frame which handles signal emission
    await get_tree().process_frame

    # Ensure display damage has been updated after damage buff
    var got_buffed_damage_display_text: String = test_random_tower_build_hud.SELECTED_TOWER_STATS_CONTAINER.TOWER_ATTR_CONTAINER.DAMAGE_VALUE.text
    assert_eq(got_buffed_damage_display_text, TEST_BUFFED_DAMAGE_DISPLAY_TEXT, "Display damage with buff amount is correct")

    # Remove damage aura from map
    test_damage_aura.queue_free()
    ...
    # Ensure tower's damage has been restored to initial value
    assert_eq(black_marble_lvl_1.PRIMARY_PROJECTILE_SPAWNER.get_damage(), TEST_STARTING_DAMAGE)

    # Simulate tower being selected again after damage buff has been removed
    test_game_map.tower_selected.emit(black_marble_lvl_1)
    # Process frame which handles signal emission
    await get_tree().process_frame

    # Ensure display damage has been restored to original value
    got_initial_display_text = test_random_tower_build_hud.SELECTED_TOWER_STATS_CONTAINER.TOWER_ATTR_CONTAINER.DAMAGE_VALUE.text
    assert_eq(got_initial_display_text, TEST_STARTING_DAMAGE_DISPLAY_TEXT, "Display damage without buff amount is correct")
