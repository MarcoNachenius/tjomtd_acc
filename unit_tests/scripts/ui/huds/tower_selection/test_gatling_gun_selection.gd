...
func test_gatling_gun_lvl_1_selection():
	# ================================================================
	#                     ** CREATE BLANK MAP **
	# ================================================================
	...
	# Create towers required for upgrade to gatling_gun
	var gatling_gun_lvl_1: Tower = TowerConstants.UPGRADE_TOWER_PRELOADS[TowerConstants.UpgradeTowerIDs.GATLING_GUN_LVL_1].instantiate()
	test_game_map.add_child(gatling_gun_lvl_1)
	gatling_gun_lvl_1.switch_state(Tower.States.BUILT)

	# Create simulated list of towers and assign to game map
	var test_towers_awaiting_selection: Array[Tower] = [gatling_gun_lvl_1]
	test_game_map.__towers_on_map = test_towers_awaiting_selection

	assert_not_null(gatling_gun_lvl_1.PRIMARY_PROJECTILE_SPAWNER, "Tower has primary projectile spawner assigned.")

	# Simulate tower being selected
	test_game_map.tower_selected.emit(gatling_gun_lvl_1)
	...
	gatling_gun_lvl_1.queue_free()
	# Process frame to ensure all objects are cleaned up
	await get_tree().process_frame

func test_gatling_gun_lvl_2_selection():
	# ================================================================
	#                     ** CREATE BLANK MAP **
	# ================================================================
	var test_game_map = GameMap.new()
	...
	# Create towers required for upgrade to gatling_gun
	var gatling_gun_lvl_2: Tower = TowerConstants.UPGRADE_TOWER_PRELOADS[TowerConstants.UpgradeTowerIDs.GATLING_GUN_LVL_2].instantiate()
	test_game_map.add_child(gatling_gun_lvl_2)
	gatling_gun_lvl_2.switch_state(Tower.States.BUILT)

	# Create simulated list of towers and assign to game map
	var test_towers_awaiting_selection: Array[Tower] = [gatling_gun_lvl_2]
	test_game_map.__towers_on_map = test_towers_awaiting_selection

	assert_not_null(gatling_gun_lvl_2.PRIMARY_PROJECTILE_SPAWNER, "Tower has primary projectile spawner assigned.")

	# Simulate tower being selected
	test_game_map.tower_selected.emit(gatling_gun_lvl_2)
