...
func test_speed_slate_lvl_1_button_use():
    # ================================================================
    #                     ** CREATE BLANK MAP **
    # ================================================================
    ...
    test_game_map.__build_tower_preload = TowerConstants.BUILD_TOWER_PRELOADS[TowerConstants.BuildTowerIDs.SPINEL_LVL_2]
    ...
    var expected_visible_tower_buttons: Array[Button] = [
        test_random_tower_build_hud.AWAITING_SELECTION_SLATE_UPGRADES_CONTAINER.SPEED_SLATE_LVL_1_BUTTON
    ]
    ...
    test_random_tower_build_hud.AWAITING_SELECTION_SLATE_UPGRADES_CONTAINER._on_speed_slate_lvl_1_button_pressed()
    # Process frame to convert selected tower to slate
    await get_tree().process_frame

    # Ensure slate with the desired ID has been added to map
    assert_eq(test_game_map.SLATE_MANAGER.__slates_on_map[0].SLATE_ID, SlateConstants.SlateIDs.SPEED_SLATE_LVL_1, "SPEED_SLATE_LVL_1 slate successfully created")

    # Clean up
    test_random_tower_build_hud.queue_free()


func test_speed_slate_lvl_2_button_use():
    # ================================================================
    #                     ** CREATE BLANK MAP **
    # ================================================================
    var test_game_map = GameMap.new()
    ...
    # BISMUTH_LVL_4
    # Simulate autoload being assigned to required tower
    test_game_map.__build_tower_preload = TowerConstants.BUILD_TOWER_PRELOADS[TowerConstants.BuildTowerIDs.BISMUTH_LVL_4]
    # Simulate tower being placed on map (placement grid coord is arbitrary)
    test_game_map.place_tower(Vector2i(5, 7))
    await get_tree().process_frame
    ...
    # Ensure intended upgrade slate buttons are visible
    var expected_visible_tower_buttons: Array[Button] = [
        test_random_tower_build_hud.AWAITING_SELECTION_SLATE_UPGRADES_CONTAINER.SPEED_SLATE_LVL_2_BUTTON
    ]
    for button in expected_visible_tower_buttons:
        assert_true(button.visible, "Intended button is visible")
    ...
    # Simulate tower slate being kept
    test_random_tower_build_hud.AWAITING_SELECTION_SLATE_UPGRADES_CONTAINER._on_speed_slate_lvl_2_button_pressed()
    # Process frame to convert selected tower to slate
    await get_tree().process_frame

    # Ensure slate with the desired ID has been added to map
    assert_eq(test_game_map.SLATE_MANAGER.__slates_on_map[0].SLATE_ID, SlateConstants.SlateIDs.SPEED_SLATE_LVL_2, "SPEED_SLATE_LVL_2 slate successfully created")

    # Clean up
    test_random_tower_build_hud.queue_free()


func test_speed_slate_lvl_3_button_use():
    # ================================================================
    #                     ** CREATE BLANK MAP **
    # ================================================================
    var test_game_map = GameMap.new()
    ...
    # KUNZITE_LVL_4
    # Simulate autoload being assigned to required tower
    test_game_map.__build_tower_preload = TowerConstants.BUILD_TOWER_PRELOADS[TowerConstants.BuildTowerIDs.KUNZITE_LVL_4]
    # Simulate tower being placed on map (placement grid coord is arbitrary)
    test_game_map.place_tower(Vector2i(5, 5))
    await get_tree().process_frame
    # LARIMAR_LVL_5
    # Simulate autoload being assigned to required tower
    test_game_map.__build_tower_preload = TowerConstants.BUILD_TOWER_PRELOADS[TowerConstants.BuildTowerIDs.LARIMAR_LVL_5]
    # Simulate tower being placed on map (placement grid coord is arbitrary)
    test_game_map.place_tower(Vector2i(5, 7))
    await get_tree().process_frame
    ...
    # Ensure intended upgrade slate buttons are visible
    var expected_visible_tower_buttons: Array[Button] = [
        test_random_tower_build_hud.AWAITING_SELECTION_SLATE_UPGRADES_CONTAINER.SPEED_SLATE_LVL_3_BUTTON
    ]
    for button in expected_visible_tower_buttons:
        assert_true(button.visible, "Intended button is visible")
    ...
    # Simulate tower slate being kept
    test_random_tower_build_hud.AWAITING_SELECTION_SLATE_UPGRADES_CONTAINER._on_speed_slate_lvl_3_button_pressed()
    # Process frame to convert selected tower to slate
    await get_tree().process_frame

    # Ensure slate with the desired ID has been added to map
    assert_eq(test_game_map.SLATE_MANAGER.__slates_on_map[0].SLATE_ID, SlateConstants.SlateIDs.SPEED_SLATE_LVL_3, "SPEED_SLATE_LVL_3 slate successfully created")
