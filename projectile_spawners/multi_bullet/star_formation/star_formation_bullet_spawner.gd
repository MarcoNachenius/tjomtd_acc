...
func _launch_projectiles():
    ...
    if __formation_policy == FormationPolicy.TARGETED:
        ...
        var first_bullet: Projectile = ProjectileConstants.BULLET_PATHS[BULLET_PRELOAD].instantiate()
        ...
        for i in range(1, launch_angles.size()):
            ...
            var new_bullet: Projectile = ProjectileConstants.BULLET_PATHS[BULLET_PRELOAD].instantiate()
